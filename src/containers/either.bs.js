...
var E = Higher$CreedOopPatterns.Newtype2({ });
...
function functor_e(param) {
  if (!class_tables[0]) {
    ...
    var inh = CamlinternalOO.inherits($$class, 0, ["fmap"], ["leftFmap"], Functor$CreedOopPatterns.functor_, false);
    var obj_init = inh[0];
  ...
  return Curry._1(class_tables[0], [
              undefined,
              Functor$CreedOopPatterns.functor_[3]
            ]);
}
...
function applicative_e(param) {
  if (!class_tables$1[0]) {
    ...
    var pure = ids[0];
    var apply = ids[1];
    var inh = CamlinternalOO.inherits($$class, 0, shared, ["liftA2"], Applicative$CreedOopPatterns.applicative_, false);
    var obj_init = inh[0];
  ...
  return Curry._1(class_tables$1[0], [
              undefined,
              Applicative$CreedOopPatterns.applicative_[3]
            ]);
}
...
function monad_e(param) {
  if (!class_tables$2[0]) {
    ...
    var inh = CamlinternalOO.inherits($$class, 0, ["bind"], [
          "return",
          "rightBind"
        ], Monad$CreedOopPatterns.monad_, false);
    var obj_init = inh[0];
  ...
  return Curry._1(class_tables$2[0], [
              undefined,
              Monad$CreedOopPatterns.monad_[3]
            ]);
}
